...
public class CharacterMovement : MonoBehaviour
{
    ...
    [SerializeField] private float baseRunSpeed = 10f;     // Base speed when running
    [SerializeField] private float rotationSpeed = 10f;   // Speed at which the character rotates

    // ============================== Jump Settings =================================
    [Header("Jump Settings")]
    [SerializeField] private float baseJumpForce = 3f;      // minimum jump force
    [SerializeField] private float maxJumpForce = 10f;      // maximum jump force after full charge
    [SerializeField] private float maxChargeTime = 3f;      // max charge time in seconds
    ...
    [SerializeField] private float baseDoubleJumpForce = 5f;      // double jump force
    ...
    // For when we interact with the powerups
    public void OnCollisionEnter(Collision other)
    {
        if(other.gameObject.tag == "speed")
        {
            speedMultiplier = 2.0f;
            Invoke("stopDoubleSpeed",5f);
            Destroy(other.gameObject);
        }
        if (other.gameObject.tag == "jump")
        {
            hasDoubleJump = true;
            Invoke("stopDoubleJump", 30f);
            Destroy(other.gameObject);
        }
    ...
    private void HandleJump()
    {
        ...
        else if (!IsGrounded && jumpRequest && hasDoubleJump && !isDoubleJumping)
        {
            rb.AddForce(Vector3.up * baseDoubleJumpForce, ForceMode.Impulse);
            jumpRequest = false;
            isDoubleJumping = true;

            if (doubleJumpSound != null && isDoubleJumping)
        ...
        if (((isCharging && !Input.GetKey(KeyCode.Space)) || chargeTime >= maxChargeTime) && IsGrounded) 
        {

            //Debug.Log("jump successful");
            isCharging = false;
            float chargeRatio = Mathf.Clamp01(chargeTime / maxChargeTime);
            float jumpForce = Mathf.Lerp(baseJumpForce, maxJumpForce, chargeRatio);
